...
export const selectFirstExamSubjects = (
  ...
  universities.map((university) => {
    if (university.second_exam_subjects === undefined) {
      console.log('共通テストの試験がありません')
      //returnUniversities.push(...universities)
    } else {
      let arrangeKey: string
      const arrayMatchKeys: string[] = []
      if (university.first_exam_subjects !== undefined) {
        for (const [key] of Object.entries(university.first_exam_subjects[0])) {
        ...
        //英・国が入っている
        console.log(
          `共通テストのcopyCheckedItemsArr:::::${university.university}大学${university.faculty}学部${university.department}学科:::::${copyCheckedItemsArr}`,
        )
        //数・理・社・情が選ばれていない
        console.log(
          `共通テストのfilterKeysの選択されていない科目（配列を空にする前）::::::${university.university}大学${university.faculty}学部${university.department}学科:::::${filterKeys}`,
        )
        ...
        arrayMatchKeys.map((elm) => {
          ...
          const altStr = elm + '_alternative'
          console.log(
            Object.keys(university.first_exam_subjects[0]).indexOf(altStr),
          )
          if (copyCheckedItemsArr.indexOf(altStr) !== -1) {
            ...
            console.log(
              `現在の配列内filterKeysの要素数:::::${filterKeys.length}`,
            )
          }
        })
